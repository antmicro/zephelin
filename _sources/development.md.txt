...
Both scripts work on the patch set of one Zephyr version, and they take which one from `$ZPL_PATCH_BASE` rather than guessing, so source `./scripts/zephyr_version.sh` script first.
By default it checks out `v4.3.0` release - to work on a different version, run `./scripts/switch_zephyr_version.sh <version>` first.

Before changing patches, ensure that the repositories are updated:
...
Then apply Zephyr patches using:
...
## Supporting a new Zephyr version

Each supported Zephyr version is described by four things, all named after the version:

* `zephyr-versions/<version>.env` - SDK URL, Python version and Renode package for CI
* `manifests/west-<version>.yml` - the West manifest
* `zephyr/patches/<version>/` - the patches
* `zephyr/patches/<version>.yml` - the patch manifest, generated by `update_patches.py`

To add a version, copy the closest existing set, adjust `revision:` and the `.env` values, then
rebase the patches:

```shell
./scripts/switch_zephyr_version.sh <new-version>   # stops on the first conflicting patch
source ./scripts/zephyr_version.sh
python scripts/apply_patches.py                    # re-apply as commits, up to the conflict
# resolve the conflicts as commits in the module checkouts, then:
python scripts/update_patches.py
```
