...
#define TVMGEN_QUANTIZED_FUNCTIONS(FUNC)		\
	FUNC(tvmgen_quantized_fused_nn_dense_subtract_add_fixed_point_multiply_add_clip_cast) \
	FUNC(tvmgen_quantized_fused_nn_dense_subtract_add_fixed_point_multiply_add_clip_cast_1) \
	FUNC(tvmgen_quantized_fused_nn_dense_subtract_add_fixed_point_multiply_add_clip_cast_2) \
	FUNC(tvmgen_quantized_fused_reshape) \
	FUNC(tvmgen_quantized_fused_reshape_1)
...
const TVMBackendPackedCFunc tvm_quantized_func_array[] = {TVMGEN_QUANTIZED_FUNCTIONS(TVMGEN_QUANTIZED_FUNC_ARRAY)};

const TVMFuncRegistry tvm_quantized_func_reg = {.names = TVMGEN_QUANTIZED_FUNCTIONS_COUNT TVMGEN_QUANTIZED_FUNCTIONS(TVMGEN_QUANTIZED_FUNC_REGISTRY) "\0",
	.funcs = tvm_quantized_func_array};

const TVMModule g_tvm_quantized_module = {&tvm_quantized_func_reg};

const TVMModule *TVMQuantizedLibEntryPoint(void) { return &g_tvm_quantized_module; }
